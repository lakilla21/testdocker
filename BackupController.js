...
class ImportController {
        ...
        this.auth.allowed('s:backup', (allowedErr, isAllowed) => {
            ...
            Fs.access('/' + backup_folder + '/', error => {
                if (error) {
                    ...
                    Fs.access('/' + backup_folder + '/' + uuid + '/' + fileName + '.zip', error => {
                        if (!error) {
                            ...
                            this.auth.server().suspend(err => {
                                ...
                                exec('cd ' + Config.get('sftp.path').toString() + '/' + uuid + folder + ' && zip -qr /' + backup_folder + '/' + uuid + '/' + fileName + '.zip *', function (createErr, stdout, stderr) {
            ...
            Fs.access('/' + backup_folder + '/' + uuid + '/' + fileName + '.zip', error => {
                if (!error) {
                    this.auth.server().suspend(err => {
                        ...
                        exec('unzip -qo /' + backup_folder + '/' + uuid + '/' + fileName + '.zip -d ' + Config.get('sftp.path').toString() + '/' + uuid + folder, function (err, stdout, srderr) {
                            auth.server().unsuspend(err => {});
    ...
    backupDelete(uuid) {
        ...
        Fs.access('/' + backup_folder + '/' + uuid + '/' + fileName + '.zip', error => {
            if (!error) {
                Fs.unlink('/' + backup_folder + '/' + uuid + '/' + fileName + '.zip', (err) => {
                    if (err) {
            ...
            if (response.statusCode === 200) {
                try {
                    ...
                    if (!_.isUndefined(json) && json.path && json.name) {
                        ...
                        const Mimetype = Mime.getType('/' + backup_folder + '/' + uuid + '/' + fileName + '.zip');
                        const Stat = Fs.statSync('/' + backup_folder + '/' + uuid + '/' + fileName + '.zip');
                        if (!Stat.isFile()) {
                        ...
                        const readStream = Fs.createReadStream('/' + backup_folder + '/' + uuid + '/' + fileName + '.zip');
